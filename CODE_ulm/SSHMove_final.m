...
%results for the estimated amplitudeMultipliers: 
%[7 13 12 30 10 50 6] (old speeds)
% [10, 10, 8, 9, 8, 30, 15] new (slower) speeds 300-1800

amplitudeMultiplier = [10, 10, 8, 9, 8, 30, 15];
%collumns 1-7 are speeds 300-4800 for 6000 distance, 8-14 for 12000 distance
%% old speeds see [300  1050   1800  2550  3300  4050  4800]
...
%% new speeds see [300   550   800   1050   1300  1550  1800]
speedIndex(1) = find(mySP(1) == 300:250:1800);
speedIndex(2) = find(mySP(2) == 300:250:1800); %which speed from 1-7  [300   550   800   1050   1300  1550  1800]

distanceIndex = (abs(myLR)/6000)-1;    %0 for 6000 myLR, 1 for 12000 myLR
whichTrial= (speedIndex+(distanceIndex*7)); %which sort of trails from 1-14
%select correct collumn as soundinput
inputSignal = mysound(:,whichTrial); % +7 for 12000 distance,+0 for 6000

% implement amplitude multipliers
for n= 1:length(speedIndex)
    inputSignal(:,n) = inputSignal(:,n).*amplitudeMultiplier(speedIndex(n));
end
...
%% flexible vibration duration dependent on movement speed and distance (old)
% the adjusted durations are already implemented in the 'create_sound_file'
% script and are not again needed here!
%theoreticalDuration = (abs(myLR/15000))./(mySP/300);
actualDuration = [0.48, 0.33, 0.28, 0.26, 0.25, 0.25, 0.25,...
   0.84, 0.51, 0.40, 0.34, 0.30, 0.29, 0.28];
duration=actualDuration(whichTrial);
...
SoundDelay = [0.07 0.07]+0.1;       %0.07 from timestamp analysis, constant factor by "feeling"
        ...
        if incontact == 0
            ...
            if now2 > skip % change here
                ...
                if DVi >= forceplateThreshold % DVi = mean voltage of the last 100 voltage datapoints
                    ...
                    play_vibro(inputSignal(:, interval), VA(interval), SoundDelay(interval), actualDuration(interval));
            ...
            if(ti >= duration(interval) + 0.5)
                clear sound;    %stopping some "mute" soundfile playing in the background
                if showFigureBlank
                    figure(blankFIG)   %maybe "release finger" figure instead?
                    showFigureBlank = 0;
                end
                %             saveAccData = 0;
                if round(DVi) <= 2 %accomodate for noisy forceplate which does not reliably reach rounded 0
