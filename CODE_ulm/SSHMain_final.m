...
port = instrfind;
if  isempty(port) == 0      %onyl try to close port if there are ports found
    fclose(port());
end
close ('all')
...
prompt.nline = 1;
prompt.default = {'subject_X', '672', '1'};     %672 with 4 distance conditions
...
saveto = strcat('C:/Users/CecileScotto/Documents/MATLAB/Alessandro/DATA/Final'); % ,...
% char(settings(1)));       
...
stimuli = SSHDesign_matrix_final();    %*final version with 4 distances and 3 map conditions
% ntrials = str2double(settings(2)); % defined later!
nTrainingTrials = 10; % number of Training trials

exp.input = randomtrials(stimuli, 1);  % ramdomize order of trials, do X times the complete 168 trials
...
%ntrials = 10;      %limit trials for testing
...
%% load soundinput file - this file contains the 'mysound' audio source
% load('sound_input_final.mat');    % old speeds 300 - 4800
load('sound_input_slow_speeds.mat'); % new slower speeds 300 - 1800
...
for k = 1:ntrials
    %simplyfy the input...
    null = SSHMove_final(exp.input(k,1:2), exp.input(k,3:4),...
...
%delete the training trials that somehow get included as zeros
if nTrainingTrials > 0
    output=output(nTrainingTrials+1:end,:); 
end
