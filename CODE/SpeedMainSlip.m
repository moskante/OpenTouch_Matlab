...
%code control the belt device (Motor: Faulhaber 3564K024B CS). In each
%trial, the motor moves twice (reference subject_cond_sess.txtand test stimulsubject_cond_sess.txtus). The order of
%the two stimuli is counterbalanced acros\s trials.
...
%09122016 moves at the first touch
%30.12.2016 Simplified design matrix
%% Prompt
...
saveto = strcat('C:/Users/FSL/Documents/MATLAB/tactile_gravity/DATA/slipV/',...
    char(settings(1)));
mkdir(saveto)
...
for k = 1:ntrials
    ...
    null = SpeedMove3(exp.input(k,9), exp.input(k,10), exp.input(k,3),...
                     exp.input(k,4:5),exp.input(k,6:7),exp.input(k, 8),...
                     index, blank, p1, uno, accelerometer, k);
%(for previous version of design_matrix() reference and comparison in col 1
%and col2.
%     null = SpeedMove3(exp.input(k,1), exp.input(k,2), exp.input(k,3),...
%                      exp.input(k,4:5),exp.input(k,6:7),exp.input(k, 8),...
%                      index, blank, p1, uno, accelerometer, k);
    pause(1);
