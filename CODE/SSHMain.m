...
saveto = strcat('C:/Users/FSL/Documents/MATLAB/tactile_gravity/DATA/SSH/',...
    char(settings(1)));
mkdir(saveto)
...
set(waitstart, 'Position', [0 0 screen_size(3) screen_size(4)], ...
    'DockControls', 'off','Menubar', 'none', 'Pointer', 'circle')
axis off
...
myformat = '%+4d\t %+4d\t %3d\t %3d\t %3d\t %3d\t %2.1f\t %2.1f\t %3.0f\t %3.0f\t %u\t %4.3f\n';   
...
for k = 1:ntrials
    ...
    %read n-1 position
    %simplyfy the input...
    null = SSHMove(exp.input(k,1:2), exp.input(k,3:4),...
    ...
    output = [exp.input(k, 1:8) POS0 LR exp.resp(k, 1:2)];
