...
% edit by felix traub 1.9.2017 (no tractor!)
...
% interval decides whether the first or second set of stimuli is to be used
interval = 1; % start with 2nd interval!  -> only do one!
% tk = 0;
ti = 0;
...
%     tempData = str2num(tempData);
    ...
    if incontact == 0
        ...
        if(ti >= Vduration(interval) + 0.55)  %the last added constant allows for an increased acc data recording window
%             if showFigureBlank
...
%cut the first 0.35sec of accData
timeIndex350ms = find(accData(:,2)>=0.35,1,'first');
accData = accData(timeIndex350ms:end,:);
...
sampleFreq = length(accData)/(accData(end,2)-accData(1,2))   %for only one interval     
% sampleFreq = length(accData)/accDataTimespan      %for 2 intervals
